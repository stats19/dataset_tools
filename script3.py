...
cur_sqlite.execute(
    '''SELECT home_team_api_id, away_team_api_id, match_api_id, goal, shoton, shotoff, foulcommit, card, cross, corner, possession  FROM "Match" WHERE match_api_id=%s;''' % (match_api_id))
...
cur_mysql.execute('''SELECT match_api_id FROM pa19_clean.match;''')
mysql_matches_api_id = cur_mysql.fetchall()
list_match_mysql = []
for mysql_match_api_id in mysql_matches_api_id:
    list_match_mysql.append(mysql_match_api_id[0])
print("end list_match_mysql")
...
for sqlitedata in match_info:
    ...
    for attrib in goal_search:
        ...
        print(
            "INSERT INTO `shots` (scorerId, assist, event_incident_typefk, elapsed, type, goalType, scored, onTarget) VALUES (%s, %s, %s, %s, \"%s\", \"%s\", %s, %s) WHERE teams_match_id = %s;" % (
                ...
                final_goal_dico.get("match_api_id")
            ))
    ...
    for attrib in shoton_search:
        ...
        if final_shoton_dico.get("player1"):
            print(
                "INSERT INTO `shots` (scorerId, event_incident_typefk, elapsed, type, scored, onTarget) VALUES (%s, %s, %s, \"%s\", %s, %s) WHERE teams_match_id = %s" % (
                    ...
                    final_shoton_dico.get("match_api_id")
                ))
    ...
    for attrib in shotoff_search:
        ...
        # substitutesdico = {}
        # substitutesdico["substitute"] = dico.get("substitute")
        # substitutesdico["team_match_id"] = dico.get("match_api_id")

        if final_shotoff_dico.get("substitute"):
            substitutesdico = {}
            substitutesdico["player"] = final_shotoff_dico.get("substitute")
            substitutesdico["team"] = final_shotoff_dico.get("team")
            substitute_player_list.append(substitutesdico)

        if final_shotoff_dico.get("player1"):
            print(
                "INSERT INTO `shots` (scorerId, event_incident_typefk, elapsed, type, scored, onTarget) VALUES (%s, %s, %s, \"%s\", %s, %s) WHERE teams_match_id = %s" % (
                    int(final_shotoff_dico.get("shooter_id")),
                    int(final_shotoff_dico.get("event_incident_typefk")),
                    int(final_shotoff_dico.get("elapsed")),
                    final_shotoff_dico.get("subtype"),
                    int(final_shotoff_dico.get("scored")),
                    int(final_shotoff_dico.get("onTarget")),
                    int(final_shotoff_dico.get("match_api_id"))
                ))
    ...
    for attrib in foulcommit_search:
        ...
        if final_foulcommit_dico.get("player1"):
            print("INSERT INTO `fouls` (event_incident_typefk, elapsed, culprit_id, victim_id, type, card) VALUES (%s, %s, %s, %s, \"%s\", %s) WHERE teams_match_id = %s" %(
                ...
                final_foulcommit_dico.get("match_api_id")
            ))
    ...
    for dico in listcard:
        ...
        if final_foulcommit_dico.get("player1"):
            print(
                "INSERT INTO `fouls` (event_incident_typefk, elapsed, culprit_id, victim_id, type, card) VALUES (%s, %s, %s, %s, \"%s\", %s) WHERE teams_match_id = %s" % (
                    final_foulcommit_dico.get("event_incident_typefk"),
                    final_foulcommit_dico.get("elapsed"),
                    final_foulcommit_dico.get("culprit_id"),
                    final_foulcommit_dico.get("victim_id"),
    ...
    for attrib in shoton_search:
        ...
        if final_cross_dico.get("player1"):
            "teams_matches_players_id, elapsed, type, event_incident_typefk"
            print(
                "INSERT INTO `crosses` (teams_matches_players_id, elapsed, type, event_incident_typefk) VALUES (%s, %s, \"%s\", %s) WHERE teams_match_id = %s" % (
                    ...
                    final_cross_dico.get("match_api_id")
                ))
    ...
    for attrib in shoton_search:
        ...
        if final_corner_dico.get("player1"):

            print(
                "INSERT INTO `corners` (teams_matches_players_id, elapsed, type) VALUES (%s, %s, \"%s\") WHERE teams_match_id = %s" % (
    ...
    for attrib in possession_search:
        ...
        for field in attrib:
            if field.tag != "stats":
                dico[field.tag] = field.text
        final_foulcommit_dico = possessionInfotoMysql(dico, connmysql)

        if final_foulcommit_dico:
            print(final_foulcommit_dico)
            print("UPDATE teams_matches SET possession = %s WHERE teams_match_id_home = %s;" % (int(final_foulcommit_dico.get("awaypos")),
                                                                                                int(final_foulcommit_dico.get("team_match_id_home"))))
            print("UPDATE teams_matches SET possession = %s WHERE teams_match_id_home = %s;" % (int(final_foulcommit_dico.get("homepos")),
                                                                                                int(final_foulcommit_dico.get("team_mach_id_away"))))
            print("\n")
    ...
    # with open("C:/Users/Pierre/PycharmProjects/xmlscript/test3.txt", "a") as substitutesplayers:
